...
class WatchPartyProvider with ChangeNotifier {
  ...
  final WatchPartyFirebaseService _firebaseService =
      WatchPartyFirebaseService();
  ...
  String?
  _lastNavigatedVideoUrl; // Track last navigated video to prevent duplicate navigations
  ...
  bool get isHost =>
      _useFirebase ? _firebaseService.isHost : _watchPartyService.isHost;
  ...
  void _handleRoomEnded(String reason) {
    ...
    if (_currentRoom == null && _roomEndedMessage != null) {
      debugPrint(
        'WatchPartyProvider: Room already ended, ignoring duplicate call',
      );
      return;
    }
  ...
  void _showRoomEndedDialog(String reason) {
    ...
    Future.delayed(const Duration(milliseconds: 100), () {
      ...
      if (context == null) {
        debugPrint(
          'WatchPartyProvider: Context not available for room ended dialog',
        );
        return;
      }
  ...
  void _handleRoomUpdate(WatchPartyRoom room) {
    ...
    final videoChanged =
        _currentRoom?.videoUrl != room.videoUrl ||
    ...
    if (_useFirebase) {
      ...
      if (!_isConnected &&
          _watchPartyService.currentRoom == null &&
          _currentRoom != null) {
        // Host ended the room - handle it similar to Firebase
        debugPrint(
          'WatchPartyProvider: Local mode - host ended room (connection lost)',
        );
    ...
    if (videoChanged && room.videoUrl.isNotEmpty && !isHost) {
      Future.delayed(const Duration(milliseconds: 200), () {
        ...
        if (_isInVideoPlayer()) {
          debugPrint(
            'WatchPartyProvider: Already in video player, player will handle video change. Skipping navigation.',
          );
          return;
        }
        ...
        if (!_isInVideoPlayer() && _currentRoom?.videoUrl == room.videoUrl) {
          debugPrint(
            'WatchPartyProvider: Not in video player, navigating guest to video ${room.videoUrl}',
          );
  ...
  void _handleVideoChange(String videoUrl, String videoTitle) {
    ...
    if (isHost) {
      debugPrint(
        'WatchPartyProvider: Host updating video, skipping navigation',
      );
      return;
    }
    ...
    if (_isInVideoPlayer()) {
      debugPrint(
        'WatchPartyProvider: Already in video player, skipping navigation. Video player will handle change.',
      );
      return;
    }
  ...
  bool _isInVideoPlayer() {
    ...
    try {
      ...
      try {
        ...
        final regularPlayer = context
            .findAncestorWidgetOfExactType<RSNewVideoPlayerScreen>();
  ...
  Future<void> _navigateToVideo(String videoUrl, String videoTitle) async {
    ...
    if (_lastNavigatedVideoUrl == videoUrl && _lastNavigationTime != null) {
      ...
      if (timeSinceLastNav.inSeconds < 3) {
        debugPrint(
          'WatchPartyProvider: Duplicate navigation prevented (same video within ${timeSinceLastNav.inSeconds}s)',
        );
        return;
      }
    }
    ...
    if (_currentRoom != null && _currentRoom!.videoUrl != videoUrl) {
      debugPrint(
        'WatchPartyProvider: Video URL mismatch, skipping navigation. Room: ${_currentRoom!.videoUrl}, Requested: $videoUrl',
      );
      return;
    }
    ...
    if (_isInVideoPlayer()) {
      debugPrint(
        'WatchPartyProvider: Already in video player, skipping navigation to $videoUrl',
      );
      return;
    }
  ...
  Future<WatchPartyRoom?> createRoom(
    ...
    if (!isSignedIn) {
      throw Exception(
        'Please sign in to create a watch party. Authentication is required for watch party features.',
      );
    }
    ...
    try {
      if (shouldUseFirebase) {
        ...
        debugPrint(
          'WatchPartyProvider: _useFirebase set to false for local mode',
        );
      ...
      if (shouldUseFirebase && useOnline) {
        debugPrint(
          'WatchParty: Firebase failed, falling back to local network',
        );
        return createRoom(
          participantName,
          useOnline: false,
  ...
  Future<WatchPartyRoom?> joinRoom(
    ...
    if (!isSignedIn) {
      throw Exception(
        'Please sign in to join a watch party. Authentication is required for watch party features.',
      );
    }
    ...
    try {
      ...
      _useFirebase = false;
      debugPrint(
        'WatchPartyProvider: Joining local network room, _useFirebase set to false',
      );
  ...
  Future<WatchPartyRoom?> joinRoomOnline(
    ...
    try {
      ...
      _useFirebase = true;
      debugPrint(
        'WatchPartyProvider: Joining online room, _useFirebase set to true',
      );
      ...
      if (room != null) {
        ...
        _connectionError =
            _firebaseService.connectionError ?? 'Failed to join room';
      }
  ...
  Future<void> leaveRoom() async {
    debugPrint(
      'WatchPartyProvider: leaveRoom() called, useFirebase: $_useFirebase',
    );
  ...
  Future<void> sendChatMessage(String message) async {
    ...
    debugPrint(
      'WatchPartyProvider: sendChatMessage called, useFirebase: $_useFirebase',
    );
    debugPrint(
      'WatchPartyProvider: isInRoom: $isInRoom, currentRoom: ${_currentRoom != null}',
    );
  ...
  Future<void> sendReaction(ReactionType type) async {
    ...
    if (participantId == null || participantId.isEmpty) {
      debugPrint(
        'WatchPartyProvider: Cannot send reaction - no participant ID',
      );
      return;
    }

    debugPrint(
      'WatchPartyProvider: sendReaction called with $type, useFirebase: $_useFirebase',
    );
    debugPrint(
      'WatchPartyProvider: isInRoom: $isInRoom, currentRoom: ${_currentRoom != null}',
    );
  ...
  void updateRoomState({
    ...
    if (!isInRoom || !isHost || _currentRoom == null) {
      debugPrint(
        'WatchPartyProvider: Cannot update room state - not host or not in room',
      );
      return;
    }
